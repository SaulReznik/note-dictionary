...
const initialState: IDictionary = JSON.parse(localStorage.getItem('note-dictionary-list') || '{}') || {};
...
const reducer: Reducer<IDictionary, IDictionaryContextAction> = (state, action) => {
    switch (action.type) {
        case 'ADD_ITEM':
        case 'EDIT_ITEM':
            localStorage.setItem('note-dictionary-list', JSON.stringify({ ...state, [action.payload.id]: action.payload }))
            return { ...state, [action.payload.id]: action.payload };
        case 'DELETE_ITEM':
            delete state[action.payload.id];
            localStorage.setItem('note-dictionary-list', JSON.stringify({ ...state }))
            return { ...state }
        default:
            return state;
    }
